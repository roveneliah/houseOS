...
export const PollCreator = ({ add, canCreate }: { add: (q: string, o: string[]) => void; canCreate: boolean }) => {
  const [question, setQuestion] = useState("");
  const [options, setOptions] = useState<string[]>(["", ""]);

  if (!canCreate) {
    return <p>Connect a wallet with $KRAUSE to create polls.</p>;
  }
...
export const PollCard = ({ poll, onVote, canVote }: { poll: Poll; onVote: any; canVote: boolean }) => {
  const [option, setOption] = useState(poll.options[0]?.id);
  const [comment, setComment] = useState("");

  const address = useUserAddress();
  const balance = useKrauseBalance(address) || "0";

  const submit = () => {
    if (!canVote) return;
    onVote(poll.id, option, { address, weight: Number(balance), comment });
    setComment("");
  };
  ...
  return (
    <div className="border p-4 rounded space-y-2 mt-4">
      ...
      {canVote ? (
        ...
        <p className="text-sm italic">Connect a wallet with $KRAUSE to vote.</p>
      )}
    </div>
  );
};
...
export default function PollsApp() {
  ...
  const address = useUserAddress();
  const balance = useKrauseBalance(address) || "0";
  const canInteract = !!address && Number(balance) > 0;

  return (
    <div className="p-4 space-y-4">
      <PollCreator add={addPoll} canCreate={canInteract} />
      {polls.map((p) => (
        <PollCard key={p.id} poll={p} onVote={vote} canVote={canInteract} />
      ))}
    </div>
  );
}
