...
const PollCreator = ({ add }: { add: (q: string, o: string[]) => void }) => {
...
const PollCard = ({ poll, onVote }: { poll: Poll; onVote: any }) => {
...
export default function PollsPage() {
  ...
  return (
    <div className="p-4 space-y-4">
      <PollCreator add={addPoll} />
      {polls.map((p) => (
        <PollCard key={p.id} poll={p} onVote={vote} />
      ))}
    </div>
  );
}
